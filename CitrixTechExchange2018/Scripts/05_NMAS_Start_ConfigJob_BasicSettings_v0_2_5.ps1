<#
.SYNOPSIS
  Connect to Citrix ADM and start a Configuration Job.
.DESCRIPTION
  Connect to Citrix ADM and start a Configuration Job, using JSON and REST API.
.NOTES
  Version:        0.2
...
Write-Host "| Run a Configuration Job on Citrix ADM with PowerShell: | " -ForegroundColor Yellow
Write-Host "------------------------------------------------------------- " -ForegroundColor Yellow
...
#		    "scheduleTimesEpoch"= "1523206800";               # used for scheduled jobs
		    ...
		    "variables"= @(
                ...
                @{"name"= "SNIP";"value"= "192.168.0.80";"display_name"= "SNIP"}
            );
		    "device_groups"= @();
		    "tz_offset"= 7200
	    }
