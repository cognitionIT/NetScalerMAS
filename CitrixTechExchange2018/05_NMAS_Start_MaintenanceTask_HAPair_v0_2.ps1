<#
.SYNOPSIS
  Connect to Citrix ADM and run a maintenance task for HA pair.
.DESCRIPTION
  Connect to Citrix ADM and run a maintenance task for HA pair, using PowerShell and NITRO.
.NOTES
  Version:        0.2
...
{
    #MAS Credentials
    $NMASCredentials = Get-Credential -Message "Enter your Citrix ADM Credentials"
}
...
Write-Host "| Start the Maintenance Task for HA Pair on Citrix ADM with PowerShell: | " -ForegroundColor Yellow
Write-Host "---------------------------------------------------------------------------- " -ForegroundColor Yellow
...
#region Start NetScaler NITRO Session
    ...
    #Connect to Citrix ADM (https is required)
...
# --------------------------------------------
# | Start the Citrix ADM Maintenance Task |
# --------------------------------------------
...
#region Add Maintenance Task for HA Pair
    ...
    Write-Host ("URI: " + $strURI) -ForegroundColor Cyan
...
#region Get Maintenance Task
    # Specifying the correct URL 
    $strURI = ("http://$NMASIP/nitro/v1/config/maintenance_job?filter=name:" + [System.Web.HttpUtility]::UrlEncode($TaskName) )
    ...
    $response.maintenance_job | Select-Object id, name, task_name, status | Format-Table -AutoSize
#endregion Get Maintenance Task


#region End NetScaler NITRO Session
    #Disconnect from Citrix ADM: To disconnect from the appliance, use the DELETE HTTP method.
